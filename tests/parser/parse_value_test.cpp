 ...
 *
 * Focused unit tests for Parser::parseValue(...) grouped under the
 * 'ParseValue' test suite. Each TEST verifies a single behavior or edge
 * case so failures are easy to diagnose.
 *
 * The tests mirror the current behavior of Parser::parseValue as implemented
 * in Parser.cpp (notably: it accepts numeric prefixes when no suffix is
 * present because std::stod parses a leading numeric portion).
 */
...
TEST(ParseValue, LowercaseSuffixNormalization)
{
  ...
  double v3 = p.parseValue("3.3e-3u", 3, ok); // mantissa '3.3e-3', suffix 'u' -> *1e-6
  EXPECT_TRUE(ok);
...
TEST(ParseValue, UnknownSuffix)
{
  ...
  bool ok2 = true;
  double v2 = p.parseValue("5FOO", 2, ok2); // 'FOO' unknown multi-letter suffix
  EXPECT_FALSE(ok2);
}
...
TEST(ParseValue, InvalidMantissaNoSuffix)
{
  ...
  // Completely non-numeric without suffix: parser will treat trailing alpha as suffix
  // and therefore report unknown suffix. Use a fresh flag per call.
  bool ok1 = true;
...
TEST(ParseValue, InvalidMantissaWithSuffix)
{
  ...
  bool ok1 = true;
  double v1 = p.parseValue("ABCK", 1, ok1); // invalid mantissa + suffix present
  EXPECT_FALSE(ok1);

  bool ok2 = true;
  double v2 = p.parseValue("K", 2, ok2); // empty mantissa with valid suffix -> invalid
  EXPECT_FALSE(ok2);
}
